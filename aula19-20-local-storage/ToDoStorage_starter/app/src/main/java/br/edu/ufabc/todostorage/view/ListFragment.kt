...
class ListFragment : Fragment() {
    ...
    inner class TaskAdapter(private val tasks: List<Task>)
         ...
         */
        inner class TaskViewHolder(itemBinding: ListItemBinding) : RecyclerView.ViewHolder(itemBinding.root) {
            /**
             * The title field.
             */
        ...
        override fun onBindViewHolder(holder: TaskViewHolder, position: Int) {
            ...
            task.tags?.sorted()?.forEach { tag ->
                LayoutInflater.from(holder.tagsContainer.context).inflate(R.layout.chip, holder.tagsContainer, false).let { view ->
                    (view as Chip).apply {
                        text = tag
                        setOnClickListener {
                            ListFragmentDirections.onTagClick(FilterCriteria.TAG, "Tag: $tag", tag).let {
                                findNavController().navigate(it)
                            }
                        }
            ...
            when (args.filterCriteria) {
                ...
                when(status) {
                    ...
                    is MainViewModel.Status.Failure -> {
                        ...
                        Snackbar.make(binding.root, "Failed to list items", Snackbar.LENGTH_LONG).show()
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.action_refresh -> viewModel.refresh().observe(this) { refreshStatus ->
                when (refreshStatus) {
                    is MainViewModel.RefreshStatus.Loading -> binding.progressHorizontal.visibility = View.VISIBLE
                    is MainViewModel.RefreshStatus.Done -> binding.progressHorizontal.visibility = View.INVISIBLE
                    is MainViewModel.RefreshStatus.Failure -> {
                        Log.e("VIEW", "Failed to refresh list", refreshStatus.e)
                        Snackbar.make(binding.root, "Failed to refresh data", Snackbar.LENGTH_LONG).show()
                        binding.progressHorizontal.visibility = View.INVISIBLE
                    }
                }
            }
